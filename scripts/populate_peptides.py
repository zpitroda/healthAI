...
def build_and_enrich_peptide_record(spec: Dict[str, Any]) -> Dict[str, Any]:
    ...
    base_record: Dict[str, Any] = {
        ...
        "t_half_numeric": PKPDEngine._parse_hours_from_string(spec.get("half_life", ""), default=6.0),
        "oral_bioavailability": spec.get("oral_bioavailability", "<1%"),
        "bioavailability_f": 0.85 if "subcut" in str(spec.get("route_of_administration", "")).lower() else 0.008,
        "volume_of_distribution": spec.get("volume_of_distribution", "0.20 L/kg"),
        "volume_of_distribution_l_kg": PKPDEngine._parse_vd_from_string(spec.get("volume_of_distribution", ""), default=0.20),
        "protein_binding": spec.get("protein_binding", "<20%"),
        "protein_binding_pct": 99.0 if ">99" in str(spec.get("protein_binding", "")) else 20.0,
        "fraction_unbound": 0.01 if ">99" in str(spec.get("protein_binding", "")) else 0.80,
        "absorption_rate_ka": 0.015 if "subcut" in str(spec.get("route_of_administration", "")).lower() else 1.0,
